// Archivo: src/LudeloCore/Discovery/ConsoleRegistry.cpp
...
Result<void> ConsoleRegistry::register_console(const DiscoveredConsole& console, const ludelo::ByteBuffer& rp_key, const std::string& credentials) {
    auto logger = spdlog::get("ludelo");
    if (logger) logger->info("Registering console {} at {}", ludelo::mask_secret(console.host_id), console.address);
...
static std::string encode_hex(const ludelo::ByteBuffer& buf) {
    std::string res;
    for (uint8_t b : buf) {
...
static ludelo::ByteBuffer decode_hex(const std::string& hex) {
    ludelo::ByteBuffer res;
...
Result<void> ConsoleRegistry::save_to_disk() {
    ...
    for (const auto& c : consoles) {
        ...
        auto enc_rp_key = ludelo::auth::Keychain::encrypt(c.rp_key);
        if (enc_rp_key) {
            obj["rp_key_enc"] = encode_hex(enc_rp_key.value());
        }
        
        if (!c.rp_auth.empty()) {
            ludelo::ByteBuffer auth_bytes(c.rp_auth.begin(), c.rp_auth.end());
            auto enc_rp_auth = ludelo::auth::Keychain::encrypt(auth_bytes);
            if (enc_rp_auth) {
    ...
    return ludelo::auth::Keychain::store_secret("consoles.json", data);
}

Result<void> ConsoleRegistry::load_from_disk() {
    auto logger = spdlog::get("ludelo");
    auto read_res = ludelo::auth::Keychain::load_secret("consoles.json");
    if (!read_res) {
    ...
    try {
        ...
        for (const auto& obj : j) {
            ...
            if (obj.contains("rp_key_enc")) {
                ...
                auto dec = ludelo::auth::Keychain::decrypt(decode_hex(hex_enc));
                if (dec) {
                    c.rp_key = dec.value();
                }
            }
