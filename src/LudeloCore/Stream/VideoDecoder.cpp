// Archivo: src/LudeloCore/Stream/VideoDecoder.cpp
...
Result<void> VideoDecoder::init(VideoCodec codec, bool hw_preferred) {
    ...
    if (!av_codec) {
        spdlog::get("ludelo")->error("Unsupported video codec");
    ...
    if (open_ret < 0) {
        spdlog::get("ludelo")->error("Failed to open codec");
        return std::unexpected(ErrorCode::DecoderInitFailed);
    }

    spdlog::get("ludelo")->info("VideoDecoder initialized with HW type: {}", m_hw_type);
    return {};
}
...
Result<DecodedFrame> VideoDecoder::decode(std::span<const uint8_t> nalu_data) {
    ...
    if (ret < 0) {
        spdlog::get("ludelo")->error("Error sending packet for decoding: {}", ret);
    ...
    while (true) {
        ...
        if (r == 0) {
            ...
            if (r != AVERROR(EAGAIN) && r != AVERROR_EOF) {
                spdlog::get("ludelo")->warn("avcodec_receive_frame error: {}", r);
            }
            break;
        }
    }
