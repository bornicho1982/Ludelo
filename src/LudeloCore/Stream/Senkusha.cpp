// Archivo: src/LudeloCore/Stream/Senkusha.cpp
#include "Senkusha.h"
...
namespace ludelo::stream {
    ...
    Result<SenkushaResult> Senkusha::run(std::shared_ptr<net::UDPSocket> socket, const std::string& remote_addr, uint16_t remote_port) {
        ...
        spdlog::get("ludelo")->info("Running Senkusha QoS negotiation to {}:{}", remote_addr, remote_port);
        ...
        spdlog::get("ludelo")->info("Senkusha finished: MTU={}/{}, RTT={}us", result.mtu_in, result.mtu_out, result.rtt_us);
