// Archivo: src/LudeloCore/Common.h
#pragma once
...
// ─── Version ──────────────────────────────────────────────
#ifndef LUDELO_VERSION
#define LUDELO_VERSION "0.1.0"
#endif
#ifndef LUDELO_USER_AGENT
#define LUDELO_USER_AGENT "Ludelo/0.1.0"
#endif

namespace ludelo {

inline constexpr std::string_view kVersion = LUDELO_VERSION;
inline constexpr std::string_view kUserAgent = LUDELO_USER_AGENT;
...
inline void init_logging(spdlog::level::level_enum level = spdlog::level::info, const std::string& log_file = "ludelo.log") {
    if (auto existing = spdlog::get("ludelo")) {
    ...
    auto logger = std::make_shared<spdlog::logger>(
        "ludelo", spdlog::sinks_init_list{console_sink, file_sink});
