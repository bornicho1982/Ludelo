// Archivo: src/LudeloCore/Cloud/GaikaiClient.cpp
...
/// Map HTTP status codes to ludelo::Error
ludelo::Error http_error(long status, const std::string& url, const std::string& body) {
    spdlog::error("[GaikaiClient] HTTP {} from {}: {}", status, url, body.substr(0, 256));
    switch (status) {
        case 401: return {ludelo::ErrorCode::TokenExpired,          "PSN access token expired or invalid"};
        case 403: return {ludelo::ErrorCode::NoPSPlusPremium,       "PS Plus Premium entitlement required"};
        case 404: return {ludelo::ErrorCode::CloudSessionFailed,    "Resource not found on Kamaji"};
        case 429: return {ludelo::ErrorCode::NetworkError,          "Rate limited by Kamaji API"};
        default:  return {ludelo::ErrorCode::CloudError,
...
ludelo::Result<bool> GaikaiClient::check_entitlement(const std::string& access_token) {
    if (access_token.empty()) {
        return std::unexpected(ludelo::Error{ludelo::ErrorCode::NoActiveAccount,
    ...
    if (response.status_code == 0) {
        return std::unexpected(ludelo::Error{ludelo::ErrorCode::NetworkError,
    ...
    try {
        ...
        if (!entitled || (tier != "premium" && tier != "ps-plus-premium")) {
            return std::unexpected(ludelo::Error{ludelo::ErrorCode::NoPSPlusPremium,
        ...
        return std::unexpected(ludelo::Error{ludelo::ErrorCode::JsonParseError, e.what()});
    }
}
...
ludelo::Result<std::vector<CloudGame>> GaikaiClient::get_catalog(const std::string& access_token) {
    if (access_token.empty()) {
        return std::unexpected(ludelo::Error{ludelo::ErrorCode::NoActiveAccount,
            "No access token provided"});
    }
...
ludelo::Result<CloudSession> GaikaiClient::create_session(
    ...
    const CloudStreamConfig& config)
{
    ...
    if (game_id.empty()) {
        return std::unexpected(ludelo::Error{ludelo::ErrorCode::InvalidParameter,
    ...
    try {
        ...
        if (session.session_id.empty() || session.streaming_endpoint.empty()) {
            ...
            return std::unexpected(ludelo::Error{ludelo::ErrorCode::CloudSessionFailed,
...
ludelo::VoidResult GaikaiClient::end_session(const std::string& session_id) {
    if (session_id.empty()) {
        return std::unexpected(ludelo::Error{ludelo::ErrorCode::InvalidParameter,
...
ludelo::Result<CloudSession> GaikaiClient::create_session(const std::string&, const std::string&, const CloudStreamConfig&) {
    return std::unexpected(ludelo::Error{ludelo::ErrorCode::CloudNotAvailable, "Cloud streaming is disabled in this build."});
}

ludelo::Result<std::vector<CloudGame>> GaikaiClient::get_catalog(const std::string&) {
    return std::unexpected(ludelo::Error{ludelo::ErrorCode::CloudNotAvailable, "Cloud streaming is disabled in this build."});
}

ludelo::Result<bool> GaikaiClient::check_entitlement(const std::string&) {
    return std::unexpected(ludelo::Error{ludelo::ErrorCode::CloudNotAvailable, "Cloud streaming is disabled in this build."});
}

ludelo::VoidResult GaikaiClient::end_session(const std::string&) {
    return {};
}
