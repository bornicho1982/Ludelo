// Archivo: src/LudeloCore/Crypto/PS5Protocol.cpp
...
static ludelo::ByteBuffer decode_hex(std::string_view hex) {
    ludelo::ByteBuffer out;
...
Result<ludelo::ByteBuffer> PS5Protocol::aes_cfb_crypt(
    ...
    ludelo::ByteBuffer output(input.size());
    int out_len = 0;
...
Result<PS5RegistrationResult> PS5Protocol::register_with_pin(
    ...
    if (logger) {
        logger->info("[PS5Protocol] Registration succeeded via Chiaki Core! Host: {}, MAC: {}",
            reg_res.host_name, ludelo::mask_secret(reg_res.mac));
    }
...
Result<PS5SessionInitResult> PS5Protocol::init_session(
    ...
    auto logger = spdlog::get("ludelo");
    if (logger) logger->info("PS5Protocol: Initiating session with {}:{} using regist_key {}", host, port, ludelo::mask_secret(regist_key));
...
PS5SessionKeys PS5Protocol::derive_session_keys(
    const ludelo::ByteBuffer& nonce,
    const ludelo::ByteBuffer& rp_key
) {
    PS5SessionKeys keys{};
