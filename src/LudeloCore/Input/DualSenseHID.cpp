...
std::vector<DualSenseInfo> DualSenseHID::enumerate() {
    ...
    if (dev_info == INVALID_HANDLE_VALUE) {
        spdlog::get("ludelo")->error("Failed to get HID device info set");
        return controllers;
    }
            ...
            if (device != INVALID_HANDLE_VALUE) {
                ...
                if (HidD_GetAttributes(device, &attrib)) {
                    if (attrib.VendorID == SONY_VID && (attrib.ProductID == DUALSENSE_PID || attrib.ProductID == DUALSENSE_EDGE_PID)) {
                        ...
                        spdlog::get("ludelo")->info("Found DualSense: {}", info.device_path);
                    }
                }
                CloseHandle(device);
            }
...
ludelo::VoidResult DualSenseHID::open(const std::string& device_path) {
    ...
    if (m_device == INVALID_HANDLE_VALUE) {
        ...
        return std::unexpected(ludelo::Error{ludelo::ErrorCode::HIDOpenFailed, "Failed to open DualSense device"});
    }
    ...
    spdlog::get("ludelo")->info("DualSense opened. Bluetooth: {}", m_is_bluetooth);
    return {};
}
...
ludelo::Result<DualSenseState> DualSenseHID::poll() {
    if (!is_open()) return std::unexpected(ludelo::Error{ludelo::ErrorCode::InputError, "Device not open"});
    ...
    if (!ReadFile(m_device, report.data(), (DWORD)report.size(), nullptr, &ol)) {
        if (GetLastError() != ERROR_IO_PENDING) {
            ...
            return std::unexpected(ludelo::Error{ludelo::ErrorCode::InputError, "ReadFile failed"});
        }
        if (!GetOverlappedResult(m_device, &ol, &bytes_read, TRUE)) {
            CloseHandle(ol.hEvent);
            return std::unexpected(ludelo::Error{ludelo::ErrorCode::InputError, "GetOverlappedResult failed"});
        }
    } else {
    ...
    if (bytes_read == 0) return std::unexpected(ludelo::Error{ludelo::ErrorCode::InputError, "Zero bytes read"});

    size_t offset = 0;
    if (m_is_bluetooth) {
        if (report[0] != 0x31) return std::unexpected(ludelo::Error{ludelo::ErrorCode::InputError, "Invalid BT report ID"});
        offset = 1;
    } else {
        if (report[0] != 0x01) return std::unexpected(ludelo::Error{ludelo::ErrorCode::InputError, "Invalid USB report ID"});
    }
...
ludelo::VoidResult DualSenseHID::send_output_report() {
    if (!is_open()) return std::unexpected(ludelo::Error{ludelo::ErrorCode::InputError, "Device not open"});
...
ludelo::VoidResult DualSenseHID::set_vibration(uint8_t left_low, uint8_t right_high) {
...
ludelo::VoidResult DualSenseHID::set_lightbar(uint8_t r, uint8_t g, uint8_t b) {
    m_lightbar_r = r;
    m_lightbar_g = g;
    m_lightbar_b = b;
    m_output_dirty = true;
...
ludelo::VoidResult DualSenseHID::set_trigger_effect(bool left_trigger, const TriggerEffect& effect) {
...
ludelo::VoidResult DualSenseHID::set_player_leds(uint8_t leds_mask) {
