#include "PortalCore/Net/TLSSocket.h"
...
TLSSocket::TLSSocket() {
    ...
    if (!ctx_) {
        spdlog::get("portal")->error("Failed to create SSL context");
    }
...
Result<void> TLSSocket::handshake() {
    ...
    if (SSL_connect(ssl_) != 1) {
        spdlog::get("portal")->error("SSL connect failed for {}", host_);
        return std::unexpected(Error{ErrorCode::NetworkError});
    }
    return {};
}
...
Result<void> TLSSocket::send_all(std::span<const uint8_t> data) {
    ...
    while (total_sent < data.size()) {
        ...
        if (sent <= 0) {
            spdlog::get("portal")->error("SSL write failed");
            return std::unexpected(Error{ErrorCode::NetworkError});
        }
        total_sent += sent;
    }
...
Result<ByteBuffer> TLSSocket::receive(size_t max_bytes) {
    ...
    if (received <= 0) {
        ...
        spdlog::get("portal")->error("SSL read failed");
        return std::unexpected(Error{ErrorCode::NetworkError});
    }
