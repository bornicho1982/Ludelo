// Archivo: src/PortalCore/Input/DualSenseHID.h
#pragma once
...
public:
    ...
    portal::VoidResult open(const std::string& device_path);
    void close();
    
    portal::Result<DualSenseState> poll();
    
    portal::VoidResult set_vibration(uint8_t left_low, uint8_t right_high);
    portal::VoidResult set_lightbar(uint8_t r, uint8_t g, uint8_t b);
    portal::VoidResult set_trigger_effect(bool left_trigger, const TriggerEffect& effect);
    portal::VoidResult set_player_leds(uint8_t leds_mask);
    
    bool is_bluetooth() const;
    bool is_open() const;

private:
    portal::VoidResult send_output_report();
