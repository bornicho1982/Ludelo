// Archivo: src/PortalCore/Stream/AudioDecoder.cpp
...
Result<void> AudioDecoder::init(int sample_rate, int channels) {
    ...
    if (!codec) {
        spdlog::get("portal")->error("Opus decoder not found in FFmpeg");
    ...
    if (avcodec_open2(m_decoder, codec, nullptr) < 0) {
        spdlog::get("portal")->error("Failed to open Opus codec");
        return std::unexpected(ErrorCode::DecoderInitFailed);
    }
    ...
    spdlog::get("portal")->info("AudioDecoder initialized with FFmpeg Opus, {} Hz, {} channels", sample_rate, channels);
    return {};
}
