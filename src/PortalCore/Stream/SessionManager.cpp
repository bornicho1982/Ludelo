// Archivo: src/PortalCore/Stream/SessionManager.cpp
...
VoidResult SessionManager::connect_local(const portal::discovery::RegisteredConsole& console, const StreamConfig& config) {
...
void SessionManager::disconnect() {
    ...
    if (m_takion_connection) {
        ...
        if (!m_cloud_session_id.empty()) {
            portal::cloud::GaikaiClient gaikai;
...
VoidResult SessionManager::connect_cloud(const portal::auth::PSNTokens& tokens, const std::string& game_id, const StreamConfig& config) {
    ...
    portal::cloud::GaikaiClient gaikai;
    portal::cloud::CloudStreamConfig cloud_cfg{
